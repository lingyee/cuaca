...
- **Rain Map tab** — live precipitation overlay powered by Tomorrow.io; defaults to a full-Malaysia view when no place is pinned, zooms to a ~60 km view centred on the selected place when one is set; auto-refreshes every 10 minutes; timeline slider to scrub back through the past 40 minutes in 10-minute steps with smooth cross-fade animation; colour-coded Rain Intensity legend (Light → Shower → Heavy → Torrential)
